package com.example.bluetoothlowenergy.adapter;
...
public class BluetoothLowEnergyAdapter extends BaseAdapter {
    private Context context;
    private List<BluetoothDevice> deviceList;
    private String[] deviceStateArray = { "未连接","连接中","已绑定","已连接"};
    public static int CONNECTED = 13;

    public BluetoothLowEnergyAdapter(Context context,List<BluetoothDevice> deviceList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(device.getState() == android.bluetooth.BluetoothDevice.BOND_NONE)
        {
            // 未连接
            holder.tv_blue_state.setText(deviceStateArray[0]);
        }
        else if(device.getState() == android.bluetooth.BluetoothDevice.BOND_BONDING)
        {
            // 连接中
            holder.tv_blue_state.setText(deviceStateArray[1]);
        }
        else if(device.getState() == android.bluetooth.BluetoothDevice.BOND_BONDED)
        {
            // 已绑定
