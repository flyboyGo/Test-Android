...
public class MoviePageListViewModel extends ViewModel {

    public LiveData<PagedList<Movie>> moviePagedListLiveData;

    public MoviePageListViewModel(){
        PagedList.Config config = new PagedList.Config.Builder()
                .setEnablePlaceholders(false) // 设置控件的占位,默认为true
                .setPageSize(MoviePositionalDataSource.PER_PAGE) // 设置每一页的数据的大小
                .setPrefetchDistance(2)// 设置当距离底部还有多少条数据时开始加载下一页(例如一页数据为10,屏幕显示7,剩余3,向下滑动时,当剩余2时,获取下一页数据)
                .setInitialLoadSizeHint(MoviePositionalDataSource.PER_PAGE * 2) // 设置首次加载的数量
                .setMaxSize(65536 * MoviePositionalDataSource.PER_PAGE)
                .build();
