...
public class MovieViewModel extends ViewModel {
    ...
    public MovieViewModel(){
        PagedList.Config config = new PagedList.Config.Builder()
                ...
                .setPageSize(MovieItemKeyedDataSource.PER_PAGE) // 设置每一页的数据的大小
                .setPrefetchDistance(2)// 设置当距离底部还有多少条数据时开始加载下一页(例如一页数据为10,屏幕显示7,剩余3,向下滑动时,当剩余2时,获取下一页数据)
                .setInitialLoadSizeHint(MovieItemKeyedDataSource.PER_PAGE * 2) // 设置首次加载的数量
                .setMaxSize(65536 * MovieItemKeyedDataSource.PER_PAGE)
                .build();
