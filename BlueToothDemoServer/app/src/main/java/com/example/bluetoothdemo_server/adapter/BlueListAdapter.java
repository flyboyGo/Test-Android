package com.example.bluetoothdemo_server.adapter;
...
public class BlueListAdapter extends BaseAdapter {
    ...
    private final String[] mStateArray = {"未绑定", "绑定中", "已绑定", "已连接"};
    private final String[] mBleStateArray = {"未连接", "已连接"};
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (device.state >= 10) {
            holder.tv_blue_state.setText(mStateArray[device.state-10]);
        } else {
            holder.tv_blue_state.setText(mBleStateArray[device.state]);
        }
