package com.example.bluetoothdemo_server;
...
import com.example.bluetoothdemo_server.adapter.BlueListAdapter;
import com.example.bluetoothdemo_server.enity.BlueDevice;
import com.example.bluetoothdemo_server.task.BlueAcceptTask;
import com.example.bluetoothdemo_server.task.BlueConnectTask;
import com.example.bluetoothdemo_server.task.BlueReceiveTask;
import com.example.bluetoothdemo_server.util.BluetoothUtil;
import com.example.bluetoothdemo_server.util.PermissionUtil;
...
public class BluetoothTransActivity extends AppCompatActivity implements
    ...
    private Handler mHandler = new Handler(Looper.myLooper()); // 声明一个处理器对象(主线程)
    private BluetoothAdapter mBluetooth; // 声明一个蓝牙适配器对象
    private BluetoothSocket mBlueSocket; // 声明一个蓝牙设备的套接字对象

    private static  final int mOpenCode = 1; // 是否允许扫描蓝牙设备的选择对话框返回结果代码
    ...
    private void initBlueDevice()
    {
        ...
        for (BluetoothDevice device : bondedDevices) {
            // 判断此设备的地址是否已经保存在map中,如果未保存,则实例化蓝牙设备对象，添加到list中
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        {
            case R.id.menu_is_discover_bluetooth:
                boolean result = mBluetooth.isDiscovering();
                if(result)
                {
                ...
                {
                    Toast.makeText(this,"未在搜索蓝牙设备",Toast.LENGTH_SHORT).show();
                }
                break;
            default:
                break;
